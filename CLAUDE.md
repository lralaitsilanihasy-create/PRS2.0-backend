...
- Tests : `mvnw.cmd test` — en CI (`.github/workflows/ci.yml`, chaque poussée) : `mvnw test -DexcludedGroups=word`, qui exclut les tests marqués `@Tag("word")` (**2** aujourd'hui, dans `ApercuBlocVisaTest`) ayant besoin de MS Word (`documents4j`) faute de Word sur les runners Linux ; ils restent exécutés en local. ⚠️ Le tag `word` visait aussi **tout test qui soumet une mise à jour**, dont la pièce d'historique régénère le PV du prédécesseur (donc passe par Word — `PvDocumentWordIntegrationTest`, `SoumissionMiseAJourPiecesHistoriqueIntegrationTest`, `DesignationMarcheLongueIntegrationTest` en touchent) : ⚠️ **2026-09-16 — ces trois classes n'en portent plus aucune occurrence**, relevé à la lecture des sources (`grep '@Tag(' src/test`) ; si un test y part en 400 « Le PV du dossier précédent est obligatoire » en CI et vert en local, c'est ce cas-là, et le tag est à reposer avant de rouvrir le dossier. Repères (comptage des méthodes `@Test`/`@ParameterizedTest` de `src/test`, 2026-09-16) : 974 tests en local, 972 en CI.

...
- Tests : `mvnw.cmd test` — en CI (`.github/workflows/ci.yml`, chaque poussée) : `mvnw test -DexcludedGroups=word`, qui exclut les tests marqués `@Tag("word")` (**2** aujourd'hui, dans `ApercuBlocVisaTest`) ayant besoin de MS Word (`documents4j`) faute de Word sur les runners Linux ; ils restent exécutés en local. Le tag `word` visait aussi, à l'origine, **tout test qui soumet une mise à jour**, dont la pièce d'historique régénère le PV du prédécesseur (`PvDocumentWordIntegrationTest`, `SoumissionMiseAJourPiecesHistoriqueIntegrationTest`, `DesignationMarcheLongueIntegrationTest`). **Vérifié le 2026-09-16 : ces trois classes ne portent plus le tag depuis leur création (`587aacc`, 2026-08-27) et la CI est verte depuis, sur 137 commits** — elles n'ont donc pas besoin de Word ; rien à reposer. Le javadoc de `PvDocumentWordIntegrationTest` (« les neuf tests marques `@Tag("word")` ») est périmé. Repères (comptage des méthodes `@Test`/`@ParameterizedTest` de `src/test`, 2026-09-16) : 974 tests en local, 972 en CI ; suite CI mesurée le 2026-09-16 sur `chantier/refonte-ergonomique` : 988 tests exécutés, 0 échec.

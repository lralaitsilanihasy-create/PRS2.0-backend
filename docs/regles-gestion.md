...
- ⚠️ **Audit 2026-09-14 (C2) — vues internes CNM filtrées par le serveur.** La PRMP (et son UGPM) suit
  ...
  de vérification ne figure pas dans le suivi des observations du PV (revue du 2026-09-14). Prolonge au serveur la
...
- Vérification de levée des observations [Action]
  ...
  - ⚠️ **Règle ajoutée (2026-08-15) — visibilité de la rectification** : au premier `PUT /api/saisies/ppm/{id}` de chaque cycle (dossier `EN_ATTENTE_DECISION_PRMP`), l'état des lignes **avant correction** est figé (`t_snapshot_rectif_ligne`) — la rectification modifiant la version courante **en place**, c'est le seul moyen de comparer. Le **diff du dernier cycle** (avant → après, lignes `INCHANGEE`/`MODIFIEE` par `idDetail`, mêmes champs comparés que le diff des versions) est servi par `GET /api/dossiers/{id}/diff-rectification` (même `DiffDossierDto` — le front réutilise son tableau surligné) à **tous les profils qui consultent le dossier** : tout-voyant, PRMP propriétaire, contrôleurs de la localité (vérificateur titulaire ou délégué compris). Le vérificateur voit ainsi **ce que la PRMP a réellement changé** au moment de statuer la levée. Après une nouvelle transmission d'observations puis une nouvelle rectification, c'est le **nouveau** cycle que sert le diff — l'ancien n'est plus effacé mais **archivé** (⚠️ 2026-09-06, règle suivante). Purge avec le circuit (retrait/annulation).
  - ⚠️ **Règle ajoutée (2026-09-06, demande pilote) — historique des versions à la rectification** : la version **remplacée** par une rectification n'est plus écrasée mais **archivée**, immuable, au premier `PUT /api/saisies/ppm/{id}` de chaque cycle — numéro d'ordre, date, auteur (PRMP opératrice + login réel), itération de rectification (cycle), en-tête du PPM et lignes **complètes** (bénéficiaires, lots, dates prévisionnelles). `GET /api/dossiers/{id}/versions-archivees` (liste, **vide** si jamais rectifié — 200, pas 409) et `GET …/versions-archivees/{numero}` (contenu, **lecture seule** : toute autre méthode → 405 ; trigger PostgreSQL refusant tout UPDATE) — même périmètre de lecture que le diff (tout-voyant, PRMP propriétaire, contrôleurs de la localité). La **version courante** est le dossier lui-même (numéro = versions archivées + 1). **Distinct** de `GET /api/dossiers/{id}/versions` (chaîne des **mises à jour**, chaque version étant un dossier) : la mécanique des mises à jour n'est pas réutilisée, elle créerait des dossiers fantômes. L'instantané d'avant la V18 est repris comme version **n° 1** (collections reconstituées depuis les empreintes, mode dégradé). Le `/diff-rectification` est **inchangé** (dernière version archivée vs courante). Purge avec le circuit (retrait/annulation/suppression), comme l'instantané.
